...
# Сохраняем в отдельный файл все нужные нам поля лог файла все те строки, которые идут после даты из файла , которые мы будем обрабатывать, для того чтобы не мешать записывать в основной файл лог
awk '{print $1,$4,$6,$7,$9}' access-4560-644067.log > access.log
echo "Count lines without marker time"
wc -l access.log

tail -n $line access-4560-644067.log | awk '{print $1,$4,$6,$7,$9}' > access.log
echo "Count lines with marker time"
wc -l access.log
...
awk '/(GET|POST)/{print $4}' access.log | sort -n | uniq -c | sort -n -r | head >> weblog
echo "-----------------------------------------------------------------------------------" >> weblog
